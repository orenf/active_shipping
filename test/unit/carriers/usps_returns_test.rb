...
class USPSReturnsTest < Test::Unit::TestCase
  ...
  def test_external_return_label_response_failure_should_raise_exception
    ...
    assert_raises ResponseError do
      @carrier.external_return_label_request({})
    end
  end

  def test_external_return_label_errors
    response = REXML::Document.new(xml_fixture('usps_returns/external_return_label_response_failure'))
  ...
  def test_external_return_label_response_should_return_external_label_response
    ...
    assert_equal @carrier.external_return_label_request({}).tracking_number, "9202090140694100000410"
  end
